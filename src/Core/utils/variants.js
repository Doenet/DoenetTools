...
export function getNumberOfVariants({ serializedComponent, componentInfoObjects }) {
  ...
  if (!variantControlChild) {
    ...
    if (nonBlankChildren.length === 1 && componentInfoObjects.isInheritedComponentType({
      ...
      if (results.success) {
        ...
        serializedComponent.variants.descendantVariantComponents = gatherVariantComponents({
          serializedComponents: serializedComponent.children,
          componentInfoObjects
        });
  ...
  return determineVariantsForSection({
    serializedComponent, componentInfoObjects, isDocument,
  })
...
export function determineVariantsForSection({
  ...
  if (!variantControlChild && !isDocument) {
    ...
    return BaseComponent.determineNumberOfUniqueVariants({ serializedComponent, componentInfoObjects });
  }

  let specifiedVariantNames = [];
  if (variantControlChild?.attributes.variantNames) {
    specifiedVariantNames = variantControlChild.attributes.variantNames.component.children
      .map(x => x.toLowerCase())
  }

  if (specifiedVariantNames.length !== [... new Set(specifiedVariantNames)].length) {
  ...
  let variantsToInclude = variantControlChild?.attributes.variantsToInclude?.component.children;
  if (variantsToInclude) {
    if (variantsToInclude.length === 0) {
      throw Error("Cannot specify a blank variantsToInclude attribute of a variantControl")
    }
    ...
    for (let variant of variantsToInclude) {
      if (!variantNames.includes(variant)) {
        throw Error(`Cannot include variant ${variant} as ${variant} is a not variant name`);
      }
    }
  }


  let variantsToExclude = variantControlChild?.attributes.variantsToExclude?.component.children;
  if (variantsToExclude) {
    ...
    for (let variant of variantsToExclude) {
      if (!variantNames.includes(variant)) {
        throw Error(`Cannot exclude variant ${variant} as ${variant} is not a variant name`);
      }
    }
  ...
  if (variantsToInclude) {
    variantsToInclude = variantsToInclude.filter(x => !variantsToExclude.includes(x));
  } else {
    variantsToInclude = [...variantNames].filter(x => !variantsToExclude.includes(x));
  }


  let variantsToIncludeUniqueIndices = variantsToInclude.map(variant => variantNames.indexOf(variant) + 1);
  ...
  let variantsToIncludeSeeds = variantsToIncludeUniqueIndices.map(i => variantSeeds[i - 1]);
  ...
  let uniqueVariantsIsSpecified = variantControlChild?.attributes.uniqueVariants !== undefined;

  let uniqueVariants = variantControlChild?.attributes.uniqueVariants?.primitive;
  ...
  if (uniqueVariants || !uniqueVariantsIsSpecified) {
    ...
    let BaseComponent = componentInfoObjects.allComponentClasses._base
    uniqueResult = BaseComponent.determineNumberOfUniqueVariants({ serializedComponent, componentInfoObjects });

    if (!uniqueResult.success) {
      uniqueVariants = false;
    } else {
      uniqueVariants = uniqueVariants || uniqueResult.numberOfVariants <= nVariantsSpecified;
    }
  ...
  if (numberOfVariants === 0) {
    throw Error("No variants selected based on variantsToInclude, variantsToExclude, and the number of variants available")
  }
