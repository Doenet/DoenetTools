...
export default function periodicSetEquality(expr, other, { min_elements_match = 3, match_partial = false } = {}) {

  // expr must be a discrete infinite set
  if (!is_periodic_set(expr))
    return false;
  ...
  if (is_periodic_set(other)) {
    ...
    if (match_partial) {
      ...
      return contained_in(expr.tree, other.tree, match_partial) &&
    ...
    let period = me.fromAst(['apply', 'abs', operands[0][2]]).evaluate_to_constant();
    ...
    for (let i = 1; i < n_in_list; i++) {
      if (Math.abs(number_list[i] - number_list[i - 1] - period) < 1E-10 * period) {
        n_matches++;
      } else {
        if (!match_partial) {
          return false;
    ...
    if (n_matches === n_in_list && n_ldots === 2 && n_in_list >= min_elements_match) {
      return true;
    }

    let fraction_matched = n_matches / (
      Math.max(n_in_list, min_elements_match)
    )
...
function contained_in(tree, i_set, match_partial) {
  ...
  while (true) {
    ...
    let offset_diff = me.fromAst(['+', offset, ['-', offset0]]).evaluate_to_constant();
  ...
  for (let base_p of all_ps) {
    ...
    for (let opt of options) {
      ...
      for (let j = 0; j < p; j++) {
        ...
        let offset_diff = me.fromAst(['+', offset, ['-', ['+', offset0, j]]]).evaluate_to_constant();
