...
export default function MainPanel(props) {
  ...
  const [mainPanelHeaderCtrlGrpEl, setMainPanelHeaderCtrlGrpEl] = useState(null);
  ...
  const resizeWindowHanlder = () => {
    if(headerCtrlGroupEl){
      console.log(headerCtrlGroupEl.offsetWidth, "headerCtrlGroupEl.clientWidth");
  ...
  return (
    <div style={{ width: props.fromOverlayNew ? "100vw" : props.showHideNavPanel || props.navPanelObj === null ? "calc(100% - 240px)" : "calc(100% - 480px)" }}>
      ...
      {props.fromOverlayNew === undefined && <HeaderMenuPanelContent open={showHideOverlay} onClose={() => { overlayOnClose() }} body={activeHeaderPanelContent} />}
       <div>




       {props.fromOverlayNew === undefined ? <div style={{ display: "flex ", height: "50px", borderBottom: "1px solid black" ,width:"100%"}}>
          ...
          <div style={{ display: "flex", width: props.headerMenuPanels && "300px"}}>
            ...
            {props.headerMenuPanels && <div style={{ display: 'flex', width: "240px", justifyContent: "space-between", borderLeft: "1px solid black" }}>
        ...
        <div style={{ display: showHideSupportPanel || props.showHideOverlayFromOverlayNew ? 'flex' : 'block' }}>
          <div style={{ width: showHideSupportPanel || props.showHideOverlayFromOverlayNew ? '50%' : '100%', height: '100%' }}>
            {props.responsiveControls ? <div style={{ display: "flex", height: "32px", borderBottom: "1px solid black" }}>
            ...
            <div style={{height: props.responsiveControls ? 'calc(100vh - 82px)' : 'calc(100vh - 50px)', overflow: 'scroll'}}>
               {props.children}  
            </div>
          </div>
          {showHideSupportPanel || props.showHideOverlayFromOverlayNew ? <div style={{ borderLeft: '1px solid black', width: '50%', overflow:'scroll' }}>{props.supportPanelObj}</div> : ""}
        </div>
      </div>
    </div>
  )
