...
export default function GradeSettings() {
  ...
  const [attemptsAllowedAdjustment, setAttemptsAllowedAdjustment] =
    useState(null);
  ...
  const storeDueDateOverride = async (doenetId, userId, newDateString) => {
    ...
    try {
      let { data } = await axios.get(`/api/saveDueDateInfo.php`, {
        params: { doenetId, userId, newDateString },
      });
      if (data.success) {
      ...
      <>
        ...
        <Increment
          ...
          onChange={(attemptsAdjustment) => {
            ...
            axios
              .get("/api/updateGradebookAdjustment.php", {
