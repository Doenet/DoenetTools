...
function CollectionSelectionOptions({ courseId, selectedDoenetId }) {
  let collectionNameAndDoenetIds = useRecoilValue(
  ...
  for (let [i, obj] of collectionNameAndDoenetIds.entries()) {
    if (selectedDoenetId == obj.doenetId) {
      CollectionOptionsJSX.push(
        <option selected key={`CollectionOptions${i}`} value={obj.doenetId}>
          {obj.label}
        </option>,
      );
    } else {
      CollectionOptionsJSX.push(
        <option key={`CollectionOptions${i}`} value={obj.doenetId}>
          {obj.label}
        </option>,
      );
    }
...
function PageOption({ selected, i, pageId }) {
  ...
  if (selected) {
    return (
      <option selected key={`PagesInACollection${i}`} value={pageId}>
        {pageObj.label}
      </option>
    );
  } else {
    return (
      <option key={`PagesInACollection${i}`} value={pageId}>
        {pageObj.label}
      </option>
    );
  }
...
export default function SelectedCollectionLink() {
  ...
  let { deleteItem, updateCollectionLink, updateContentLinksToSources } =
    useCourse(courseId);
  ...
  if (itemObj.collectionDoenetId) {
    ...
    for (let [i, pageId] of Object.entries(
      ...
      let selected = false;
      if (
        itemObj?.manuallyFilteredPages &&
        itemObj.manuallyFilteredPages.includes(pageId)
      ) {
        selected = true;
      }
      storedPageOptionsJSX.push(
        <PageOption selected={selected} i={i} pageId={pageId} />,
      );
    }
