...
export default function ContentByWeek({ courseId }) {
    ...
    if (data.success) {
      if (isPinned) {
        let updatedPinnedAssignments = pinnedAssignments.map((assig) => {
          return assig.doenetId === assignment.doenetId
            ? { ...assig, isCompleted: !assig.isCompleted }
            : assig;
        });
  ...
  const headerMonday = `${monday.getDate()} ${monday.toLocaleString('en-US', {
    month: 'short',
  })}`;
  const headerSunday = `${sunday.getDate()} ${sunday.toLocaleString('en-US', {
    month: 'short',
  })}`;
  ...
  useEffect(() => {
    setDisplayedAssignments(
      assignments.filter(
        (assignment) =>
          new Date(assignment.dueDate) >= monday &&
          new Date(assignment.dueDate) <= sunday,
      ),
    );
          ...
          <thead>
            <Row>
              <TableHead textAlign="left">
                <Text fontSize=".9rem">ACTIVITY NAME</Text>
              </TableHead>
              ...
              <TableHead>
                <Text fontSize=".9rem">COMPLETED</Text>
              </TableHead>
              <TableHead>
                <Text fontSize=".9rem">VIEW RELATED</Text>
              </TableHead>
            </Row>
          </thead>
          <tbody>
            {pinnedAssignments.map((assignment) => (
              <Row key={`assignment-${assignment.doenetId}`}>
                <Col
                  ...
                  onClick={() =>
                    navigate(
                      `/course?tool=editor&doenetId=${assignment.doenetId}`,
                    )
                  }
                >
