...
export default function EndExamPanel() {
  ...
  const itemWeights = useRecoilValue(searchParamAtomFamily("itemWeights"))
    .split(",")
    .map(Number);
  ...
  useEffect(() => {
    axios
      .get(`/api/loadAssessmentCreditAchieved.php`, {
  ...
  if (creditByItem.length > 0) {
    ...
    if (creditForAssignment) {
      score =
        Math.round(creditForAssignment * totalPointsOrPercent * 100) / 100;
    }
    ...
    let creditByItemsJSX = creditByItem.map((x, i) => {
      ...
      return (
        <ScoreContainer key={`creditByItem${i}`}>
          Item {i + 1}:{" "}
    ...
    scoreResults = (
      <div style={{ leftMargin: "100px", leftPadding: "100px" }}>
        <ScoreContainer>
          Possible Points:{" "}
