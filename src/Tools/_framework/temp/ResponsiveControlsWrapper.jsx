...
// console.log(">>>control groups width",ctrlGrpWidthsArray )
    ...
    if(ctrlGrpWidthsArray.length > 0 && minimizedItemIndex === -1) {
      ...
      if(totalInitialWidth > props.mainPanelWidth) {                    //checks if initial width of all control groups is more than main panel width
  ...
  useEffect(()=> {
    ...
    //while minimizing
      ...
      if(props.mainPanelWidth < verticalDividersWidth + unwrappedChildrenWidth + (wrappedChildrenIndexes.length * WRAPPED_CONTROL_GROUP_WIDTH
        ...
        toBeUpdatedChildrenArray[unwrappedChildrenIndexes[unwrappedChildrenIndexes.length-1]] = <wrappedCtrlGrp>{toBeUpdatedChildrenArray[unwrappedChildrenIndexes.length-1]["props"].icon}</wrappedCtrlGrp>;
 ...
 // creating wrap element
  function createWrappedEl(propArray,index) {
    ...
    if(propArray[index] && propArray[index]["props"] && propArray[index]["props"].icon){
      return (
        <wrappedCtrlGrp>{propArray[index]["props"].icon}</wrappedCtrlGrp>
      )
    }
  }
  ...
  return (
    <React.Fragment>
      {tobeRenderedChildren ? tobeRenderedChildren.map((obj,index)=> {
        return (
        <div> 
          {obj && obj.type && obj.type === "wrappedCtrlGrp" ? 
          <>
            <div style={{display: "flex"}} onClick = {()=>{showMinimizedVersion(obj,index)}}>
              {obj}
              {index === 0 && (tobeRenderedChildren.length - 1) ? <VerticalDivider/> : (index / 2 !== 0 && index !== (tobeRenderedChildren.length - 1)) ? <VerticalDivider/> : ""}
            </div>
            {isOpen ? <MinimizedSection style={{left: leftPos ? leftPos : 770, position:"absolute"}}>{minimizedItemIndex !==-1 ? defaultChildrens[minimizedItemIndex] : ""} </MinimizedSection> : false}
          </> : 
          <div style={{display: "flex"}}>
            {obj}
            {index === 0 && index !== (tobeRenderedChildren.length - 1) ? <VerticalDivider/> : (index / 2 !== 0 && index !== (tobeRenderedChildren.length - 1)) ? <VerticalDivider/> : ""}
            </div>
          }
        </div>
        )
