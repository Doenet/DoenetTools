...
import MenuPanelWrap from './MenuPanelWrap';
                    ...
                    obj && obj.type && typeof(obj.type) === "function" && obj.type.name !== "NavPanel" && obj.type.name !== "MenuPanel" && obj.type.name !== "SupportPanel" && (
                        <>
                            {obj.type.name === "MainPanel" ? 
                                React.cloneElement(obj,{responsiveControlsFromTools: props.responsiveControls,responsiveControls: obj.props.responsiveControls,hideNavPanel:hideNavPanel, showHideNavPanel: showHideNavPanel, onUndo: props.onUndo, onRedo: props.onRedo,title:props.title, supportPanelObj: 
                                supportPanelObj, headerMenuPanels: props.headerMenuPanels}) : obj}
                        </>
                ))
            })}
                    ...
                    {props.children && Array.isArray(props.children) && props.children.map((obj,index)=> {
                        return (
                            obj && obj.type && typeof(obj.type) === "function" && obj.type.name === "MenuPanel" && (
                                <div style={{width: "100%", borderBottom: "1px solid black", borderRight:"1px solid black" ,backgroundColor:"#e2e2e2"}}>
                                    <button onClick={()=> {showHideMenuPanelContent(index)}}>{obj.props.title}</button>
                                </div>
                            )
                    )})}
                </div>
