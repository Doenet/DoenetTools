...
// Replaced Object.keys(info["properties"]).concat(GlobalAttrNames) because complete-docs has global attrs under properties
function completeAttrName(state, tree, from, to, context) {
    ...
    for (let attrName of (info && info["properties"] ? Object.keys(info["properties"]) : GlobalAttrNames)) {
...
function completeAttrValue(state, tree, from, to, context) {
    ...
    if (attrName) {
        ...
        if (!attrs) {
            let elt = findParentElement(tree), info = elt ? Tags[elementName(state.doc, elt)] : null;
            attrs = (info === null || info === void 0 ? void 0 : info["properties"]) && info["properties"][attrName];
        }
...
export { doenetml, doenetmlCompletion, doenetmlSupport, doenetmlSyntax, attrsLookup };
