 ...
 *
 * If the revision already exists and `autoGenerated` is `false`,
 * then update `revisionName`, `note`, and `autoGenerated` for the record.
 *
 ...
 *
 * Returns a Promise that resolves to an object with the field
...
export async function createContentRevision({
  ...
  let contentRevision = await prisma.contentRevisions.findUnique({
  ...
  if (!contentRevision || !autoGenerated) {
    ...
    if (!contentRevision || lastRevisionNum === null) {
      ...
      contentRevision = await prisma.contentRevisions.update({
        where: {
          contentId_revisionNum: { contentId, revisionNum: lastRevisionNum },
        },
        data: {
          revisionName,
          note,
  ...
  return { revisionNum: contentRevision.revisionNum };
}
