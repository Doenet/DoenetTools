...
test("getPublicContentByCid only gets public activity", async () => {
  ...
  const { revisionNum: publicContentRevisionNum } = await createContentRevision(
    {
      contentId: publicContentId,
      loggedInUserId: ownerId,
      autoGenerated: true,
  ...
  const { revisionNum: privateContentRevisionNum } =
    await createContentRevision({
      contentId: privateContentId,
      loggedInUserId: ownerId,
      autoGenerated: true,
  ...
  const { cid: publicContentCid } =
    await prisma.contentRevisions.findFirstOrThrow({
      where: {
        contentId: publicContentId,
        revisionNum: publicContentRevisionNum,
      },
      select: {
        cid: true,
      },
  ...
  const { cid: privateContentCid } =
    await prisma.contentRevisions.findFirstOrThrow({
      where: {
        contentId: privateContentId,
        revisionNum: privateContentRevisionNum,
      },
      select: {
        cid: true,
      },
...
test("getPublicContentByCid gets doenetml from correct revision", async () => {
  ...
  const { revisionNum: revision2 } = await createContentRevision({
    contentId: publicContentId,
    loggedInUserId: ownerId,
    autoGenerated: true,
