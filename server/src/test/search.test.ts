...
test("searchSharedContent returns public/shared activities and folders matching the query", async () => {
  ...
  const { contentId: publicActivityId } = await createContent(
    ownerId,
    "singleDoc",
    null,
  );
  ...
  const { contentId: privateActivityId } = await createContent(
    ownerId,
    "singleDoc",
    null,
  );
  ...
  const { contentId: sharedActivityId } = await createContent(
    ownerId,
    "singleDoc",
    null,
  );
...
test("searchSharedContent returns public/shared activities and folders even in a private folder", async () => {
  ...
  const { contentId: parentId } = await createContent(ownerId, "folder", null);
...
test("searchClassificationsWithSharedContent, returns only classifications with public/shared/non-deleted content", async () => {
  ...
  const { contentId: activityIdPublic } = await createContent(
    ownerId,
    "singleDoc",
    null,
  );
...
test("searchClassificationsWithSharedContent, filter by activity feature", async () => {
  ...
  const { contentId: activityIdA1 } = await createContent(
    ownerId,
    "singleDoc",
    null,
  );
