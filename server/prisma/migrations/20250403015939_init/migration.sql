...
CREATE TABLE `contentRevisions` (
    ...
    `revisionName` VARCHAR(191) NOT NULL,
    `note` TEXT NULL,
    ...
    INDEX `contentRevisions_contentId_cid_idx`(`contentId`, `cid`),
...
CREATE TABLE `contributorHistory` (
    `remixContentId` BINARY(16) NOT NULL,
    `remixContentRevisionNum` INTEGER NOT NULL,
    `originContentId` BINARY(16) NOT NULL,
    `originContentRevisionNum` INTEGER NOT NULL,
    `withLicenseCode` VARCHAR(10) NULL,
    `timestampRemixContent` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `timestampOriginContent` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `directCopy` BOOLEAN NOT NULL DEFAULT false,
    `subscribeOriginContentChange` BOOLEAN NOT NULL DEFAULT true,
    `subscribeRemixContentChange` BOOLEAN NOT NULL DEFAULT false,

    INDEX `contributorHistory_originContentId_originContentRevisionNum_idx`(`originContentId`, `originContentRevisionNum`),
    PRIMARY KEY (`remixContentId`, `originContentId`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `contributorHistory` ADD CONSTRAINT `contributorHistory_remixContentId_remixContentRevisionNum_fkey` FOREIGN KEY (`remixContentId`, `remixContentRevisionNum`) REFERENCES `contentRevisions`(`contentId`, `revisionNum`) ON DELETE NO ACTION ON UPDATE NO ACTION;

-- AddForeignKey
ALTER TABLE `contributorHistory` ADD CONSTRAINT `contributorHistory_originContentId_originContentRevisionNum_fkey` FOREIGN KEY (`originContentId`, `originContentRevisionNum`) REFERENCES `contentRevisions`(`contentId`, `revisionNum`) ON DELETE NO ACTION ON UPDATE NO ACTION;
