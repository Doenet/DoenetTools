...
CREATE TABLE `libraryActivityInfos` (
    ...
    `status` ENUM('PENDING_REVIEW', 'REQUEST_REMOVED_BY_OWNER', 'PUBLISHED', 'NEEDS_REVISION') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `libraryActivityInfos` ADD CONSTRAINT `libraryActivityInfos_sourceActivityId_fkey` FOREIGN KEY (`sourceActivityId`) REFERENCES `content`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `libraryEvents` ADD CONSTRAINT `libraryEvents_activityId_fkey` FOREIGN KEY (`activityId`) REFERENCES `content`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
