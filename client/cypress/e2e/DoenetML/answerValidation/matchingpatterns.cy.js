    ...
    for (let varName in desiredResults) {
      ...
      for (let expr in resultsForVar) {
        ...
        if (res.correct) {
          ...
          cy.get(cesc("#\\/p_sub") + " .mjx-mrow")
            .eq(0)
            .should("have.text", res.response[1]);
          cy.get(cesc("#\\/p_quad") + " .mjx-mrow")
            .eq(0)
            .should("have.text", res.matches[0][1]);
          cy.get(cesc("#\\/p_lin") + " .mjx-mrow")
            .eq(0)
            .should("have.text", res.matches[1][1]);
          cy.get(cesc("#\\/p_const") + " .mjx-mrow")
            .eq(0)
            .should("have.text", res.matches[2][1]);
          cy.window().then(async (win) => {
            let stateVariables = await win.returnAllStateVariables1();
            expect(stateVariables["/ans"].stateValues.submittedResponse1).eqls(
              res.response[0],
            );
            expect(stateVariables["/ans"].stateValues.submittedResponse2).eqls(
              res.matches[0][0],
            );
            expect(stateVariables["/ans"].stateValues.submittedResponse3).eqls(
              res.matches[1][0],
            );
            expect(stateVariables["/ans"].stateValues.submittedResponse4).eqls(
              res.matches[2][0],
            );
          });
        } else {
          ...
          cy.window().then(async (win) => {
            ...
            expect(stateVariables["/ans"].stateValues.submittedResponse2).eq(
              undefined,
            );
            expect(stateVariables["/ans"].stateValues.submittedResponse3).eq(
              undefined,
            );
            expect(stateVariables["/ans"].stateValues.submittedResponse4).eq(
              undefined,
            );
          });
        }
