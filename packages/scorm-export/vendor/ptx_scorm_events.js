...
(function () {
    ...
    // Persisted to localStorage (fast same-device cache) AND, VENDOR-MOD
    // (doenet-scorm-export), compressed into cmi.suspend_data by
    // buildSuspendData() so state survives a fresh LMS launch where localStorage
    ...
    // VENDOR-MOD (doenet-scorm-export): budget raised for the SCORM 2004 4th
    // Edition suspend_data SPM of 64,000 chars (our manifest declares 4th Ed).
    // The compressed Doenet UI-state blob (below) is stored here too, so it
    // must fit within this budget alongside the score bookkeeping.
    var SUSPEND_TOTAL_LIMIT  = 60000;  // total budget; 4th Ed SPM is 64000, keep headroom
    ...
    function buildSuspendData() {
        ...
        // ── VENDOR-MOD (doenet-scorm-export): persist Doenet UI state ─────────
        // Compress the in-memory Doenet state map into suspend_data so the
        // student's work restores on a fresh LMS launch.  localStorage (the
        // upstream store) does not survive across LMS launches; suspend_data,
        // being part of the SCORM data model, is persisted server-side by the
        // LMS.  We compress (LZ-string) because raw Doenet blobs run ~15 KB and
        // grow with interaction; base64 keeps the payload JSON- and
        // transport-safe on every LMS.
        if (window.LZString && Object.keys(_doenetStates).length > 0) {
            try {
                slim.dz = window.LZString.compressToBase64(JSON.stringify(_doenetStates));
            } catch (e) {
                console.warn('[PTX-SCORM] Could not compress Doenet state:', e);
            }
        }

        var json = JSON.stringify(slim);
        if (json.length > SUSPEND_TOTAL_LIMIT) {
            // Over budget: first drop the (large) answer text but KEEP the Doenet
            // state blob — UI restore depends on it, while grading only needs
            // correct/percent (which stay).
            Object.keys(slim.answers).forEach(function (divId) {
                slim.answers[divId].answer = '';
            });
            json = JSON.stringify(slim);
        }
        if (json.length > SUSPEND_TOTAL_LIMIT && slim.dz) {
            // Still over budget (pathologically large / heavily-used activity):
            // drop the Doenet state blob too and fall back to localStorage-only
            // restore, rather than corrupting suspend_data (which on Blackboard
            // would also take the score persist down with it).
            console.warn('[PTX-SCORM] Doenet state (' + slim.dz.length +
                ' chars compressed) exceeds suspend_data budget ' + SUSPEND_TOTAL_LIMIT +
    ...
    // VENDOR-MOD (doenet-scorm-export): rehydrate the Doenet state map from a
    // parsed suspend_data object.  buildSuspendData() stores the whole
    // _doenetStates map as one compressed base64 string under `dz`.  Here we
    // decompress it back into _doenetStates so that loadDoenetState() (which
    // reads _doenetStates first) can answer SPLICE.getState on a fresh LMS
    // launch, where localStorage has not survived.  Merges rather than replaces
    // so an already-loaded same-device value is never clobbered by the server
    // copy.  Called from both restore paths (initSession, loadRestoreData).
    function restoreDoenetStates(savedObj) {
        if (!savedObj || !savedObj.dz || !window.LZString) return;
        try {
            var json = window.LZString.decompressFromBase64(savedObj.dz);
            if (!json) return;
            ...
            console.warn('[PTX-SCORM] Could not restore Doenet state from suspend_data:', e);
        }
    }
    ...
    function initSession() {
        ...
        if (raw) {
            try {
                ...
                restoreDoenetStates(parsed);  // VENDOR-MOD: decompress Doenet UI state
                delete _state.dz;             // don't keep the compressed blob on _state
    ...
    window.addEventListener('message', function (event) {
        ...
        try {
            recordInteraction({
                // VENDOR-MOD (doenet-scorm-export): in a Runestone-free package the
                // state-saving branch above (gated on RunestoneBase) never runs, so
                // this is the ONLY place the Doenet state blob can be captured.
                // Pass it through as `state`, and forward the real subject for a
                // reportScoreAndState so recordInteraction's Doenet init-guard fires
                // (otherwise the post-restore auto-report would corrupt the score).
                // 'SPLICE.reportScoreAndState' maps to the same SCORM type ('other')
                // as 'splice', so interaction recording is otherwise unchanged; any
                // other splice event keeps the 'splice' mapping.
                event:   data.subject === 'SPLICE.reportScoreAndState'
                             ? 'SPLICE.reportScoreAndState' : 'splice',
                div_id:  divId,
                answer:  answer,
                percent: score,
                state:   data.state
    ...
    function loadRestoreData() {
        ...
        if (raw) {
            try {
                ...
                restoreDoenetStates(saved);  // VENDOR-MOD: decompress Doenet UI state
            } catch (e) {
                console.warn('[PTX-SCORM] Could not parse suspend_data:', e);
            }
        }
