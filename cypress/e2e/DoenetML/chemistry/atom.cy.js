...
describe('Atom Tests', function () {
  ...
  `}, "*");
    ...
    cy.get(cesc("#\\/electronConfiguration") + " .mjx-mrow").eq(0).invoke('text').then((text) => {
      expect(text).eq("1s1")
    })

    cy.get(cesc("#\\/aNum") + " textarea").type("{end}{backspace}2{enter}", { force: true });
    ...
    cy.get(cesc("#\\/aNum") + " textarea").type("{home}1{enter}", { force: true });
    ...
    cy.get(cesc("#\\/aNum") + " textarea").type("{home}{rightArrow}{backspace}5{enter}", { force: true });
