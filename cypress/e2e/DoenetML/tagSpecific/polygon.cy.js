...
async function testPolygonCopiedTwice({ vertices,
  polygonName = "/pg",
  ...
  cy.window().then(async (win) => {
    ...
    for (let i in vertices) {
      if (Number.isFinite(vertices[i][0])) {
        expect(me.fromAst(stateVariables[graph1Name + polygonName].stateValues.vertices[i][0]).evaluate_to_constant()).closeTo(vertices[i][0], 1E-12);
        expect(me.fromAst(stateVariables[graph2Name + polygonName].stateValues.vertices[i][0]).evaluate_to_constant()).closeTo(vertices[i][0], 1E-12);
        expect(me.fromAst(stateVariables[graph3Name + polygonName].stateValues.vertices[i][0]).evaluate_to_constant()).closeTo(vertices[i][0], 1E-12);
      } else {
        expect(stateVariables[graph1Name + polygonName].stateValues.vertices[i][0]).eq(vertices[i][0]);
        expect(stateVariables[graph2Name + polygonName].stateValues.vertices[i][0]).eq(vertices[i][0]);
        expect(stateVariables[graph3Name + polygonName].stateValues.vertices[i][0]).eq(vertices[i][0]);
      }
      if (Number.isFinite(vertices[i][1])) {
        expect(me.fromAst(stateVariables[graph1Name + polygonName].stateValues.vertices[i][1]).evaluate_to_constant()).closeTo(vertices[i][1], 1E-12);
        expect(me.fromAst(stateVariables[graph2Name + polygonName].stateValues.vertices[i][1]).evaluate_to_constant()).closeTo(vertices[i][1], 1E-12);
        expect(me.fromAst(stateVariables[graph3Name + polygonName].stateValues.vertices[i][1]).evaluate_to_constant()).closeTo(vertices[i][1], 1E-12);
      } else {
        expect(stateVariables[graph1Name + polygonName].stateValues.vertices[i][1]).eq(vertices[i][1]);
        expect(stateVariables[graph2Name + polygonName].stateValues.vertices[i][1]).eq(vertices[i][1]);
        expect(stateVariables[graph3Name + polygonName].stateValues.vertices[i][1]).eq(vertices[i][1]);
      }
    }
...
describe('Polygon Tag Tests', function () {
  ...
  it('new polygon as translated version of polygon', () => {
    ...
    async function testPolygons({ vertices, transX, transY }) {
      ...
      cy.window().then(async (win) => {
        ...
        for (let i in vertices) {
          if (Number.isFinite(vertices[i][0])) {
            expect(me.fromAst(stateVariables["/_polygon1"].stateValues.vertices[i][0]).evaluate_to_constant()).closeTo(vertices[i][0], 1E-12);
            expect(me.fromAst(stateVariables["/_polygon2"].stateValues.vertices[i][0]).evaluate_to_constant()).closeTo(vertices2[i][0], 1E-12);
          } else {
            expect(stateVariables["/_polygon1"].stateValues.vertices[i][0]).eq(vertices[i][0]);
            expect(stateVariables["/_polygon2"].stateValues.vertices[i][0]).eq(vertices2[i][0]);
          }
          if (Number.isFinite(vertices[i][1])) {
            expect(me.fromAst(stateVariables["/_polygon1"].stateValues.vertices[i][1]).evaluate_to_constant()).closeTo(vertices[i][1], 1E-12);
            expect(me.fromAst(stateVariables["/_polygon2"].stateValues.vertices[i][1]).evaluate_to_constant()).closeTo(vertices2[i][1], 1E-12);
          } else {
            expect(stateVariables["/_polygon1"].stateValues.vertices[i][1]).eq(vertices[i][1]);
            expect(stateVariables["/_polygon2"].stateValues.vertices[i][1]).eq(vertices2[i][1]);
          }
        }
  ...
  `}, "*");
    ...
    cy.get(cesc('#\\/p1') + ' .mjx-mrow').should('contain.text', `(${x1},${y1})`)
  ...
  `}, "*");
    ...
    cy.window().then(async (win) => {
      ...
      cy.get(cesc('#\\/A2') + ' .mjx-mrow').should('contain.text', `(${nInDOM(-4.5)}`)
  ...
  it('changing styles', () => {
    ...
    cy.get(cesc('#\\/stn4')).should('have.text', 'filled green polygon with a red border')
    ...
    cy.get(cesc('#\\/stn6')).should('have.text', 'filled blue polygon with a thick border')
    ...
    cy.get(cesc('#\\/stn8')).should('have.text', 'filled green polygon with a thin dotted red border')
  ...
  it('style description changes with theme', () => {
    ...
    cy.get(cesc('#\\/Adescrip')).should('have.text', 'Polygon A is filled brown with thick border.');
    cy.get(cesc('#\\/Bdescrip')).should('have.text', 'B is a filled dark red polygon.');
    cy.get(cesc('#\\/Cdescrip')).should('have.text', 'C is a filled black polygon with a thin border.');
    ...
    cy.get(cesc('#\\/Adescrip')).should('have.text', 'Polygon A is filled yellow with thick border.');
    cy.get(cesc('#\\/Bdescrip')).should('have.text', 'B is a filled light red polygon.');
    cy.get(cesc('#\\/Cdescrip')).should('have.text', 'C is a filled white polygon with a thin border.');
