...
describe("Circle Tag Tests", function () {
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      // Note on the result of allowFlexibleMotion being false (the default).
      // The following isn't the desired behavior, but it is a result of the situation
    ...
    cy.window().then(async (win) => {
      ...
      // Note on the result of allowFlexibleMotion being false (the default).
      // Given previous radius is 7, would move through point to (4, -24),
  ...
  it("circle where center depends on through point, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
  <text>a</text>
  <graph>
    <circle through="(1,4)" center="($_circle1.throughPointX1_1, $_circle1.throughPointX1_2/2)" allowFlexibleMotion />
    <copy prop="center" source="_circle1" assignNames="centerPoint" />
  </graph>
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      let actualHeight = (5 + 2) / 2;
      // Given previous radius is 2, would move through point to (-3, 5+2),
      // so that center of circle will be (-3,(5+2)/2).
      // With allowFlexibleMotion set to true, no additional adjustments are made
      await win.callAction1({
        actionName: "moveCircle",
        componentName: "/_circle1",
        args: { center: [-3, desiredHeight] },
    ...
    cy.window().then(async (win) => {
      ...
      let actualHeight = -12;
      // Given previous radius is 7, would move through point to (4, -24),
      // so that center of circle will be (4,-12).
      // With allowFlexibleMotion set to true, no additional adjustments are made
      await win.callAction1({
        actionName: "moveCircle",
        componentName: "/_circle1",
  ...
  it("circle where through point depends on center, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
  <text>a</text>
  <graph>
    <circle through="($_circle1.centerX1,$(_circle1.centerX2)2)" center="(1,2)" allowFlexibleMotion />
    <copy prop="center" source="_circle1" assignNames="centerPoint" />
  </graph>
  ...
  it("circle with center and through point, center constrained, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
    ...
    <circle center="$_point1" through="$_point2" allowFlexibleMotion />
    </graph>
    <graph>
    <copy prop="center" assignNames="centerPoint" source="_circle1" />
    <point x="$(_circle1.radius)" y="0" />
  ...
  it("circle with center and through point, through point constrained, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
  ...
  it("circle through two points, one point constrained, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
    ...
    <point>(3,4)</point>
    <circle through="$_point1 $_point2" allowFlexibleMotion />
    </graph>
    <graph>
    <copy prop="center" assignNames="centerPoint" source="_circle1" />
    <point x="$(_circle1.radius)" y="0" />
  ...
  it("circle through three points, one point constrained, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
    ...
    <point>(-3,4)</point>
    <circle through="$_point1 $_point2 $_point3" allowFlexibleMotion />
    </graph>
    <graph>
    <copy prop="center" assignNames="centerPoint" source="_circle1" />
    <point x="$(_circle1.radius)" y="0" />
    </graph>
    <copy prop="throughPoints" source="_circle1" assignNames="tp1 tp2 tp3" />
    <graph name="graph3" newNamespace>
      <copy assignNames="circle" source="../_circle1" />
    </graph>
    <copy assignNames="graph4" source="graph3" />
    ...
    cy.window().then(async (win) => {
      ...
      cy.get(cesc("#\\/tp1") + " .mjx-mrow").should(
        "contain.text",
        `(${nInDOM(Math.trunc(t1x * 100) / 100)}`,
      );
      cy.get(cesc("#\\/tp1") + " .mjx-mrow").should(
        "contain.text",
        `${nInDOM(Math.trunc(t1y * 100) / 100)}`,
      );
      cy.get(cesc("#\\/tp2") + " .mjx-mrow").should(
        "contain.text",
        `(${nInDOM(Math.trunc(t2x * 100) / 100)}`,
      );
      cy.get(cesc("#\\/tp2") + " .mjx-mrow").should(
        "contain.text",
        `${nInDOM(Math.trunc(t2y * 100) / 100)}`,
      );
      cy.get(cesc("#\\/tp3") + " .mjx-mrow").should(
        "contain.text",
        `(${nInDOM(Math.trunc(t3x * 100) / 100)}`,
      );
      cy.get(cesc("#\\/tp3") + " .mjx-mrow").should(
        "contain.text",
        `${nInDOM(Math.trunc(t3y * 100) / 100)}`,
      );
    });
    ...
    cy.window().then(async (win) => {
      ...
      t1x = Math.round(t1x * 1e14) / 1e14;
      t1y = Math.round(t1y * 1e14) / 1e14;
      t2x = Math.round(t2x * 1e14) / 1e14;
      t2y = Math.round(t2y * 1e14) / 1e14;
      t3x = Math.round(t3x * 1e14) / 1e14;
      t3y = Math.round(t3y * 1e14) / 1e14;
  ...
  it("circle through three points, two points constrained, allow flexible motion", () => {
    cy.window().then(async (win) => {
      win.postMessage(
        {
          doenetML: `
    ...
    cy.window().then(async (win) => {
      ...
      cy.get(cesc("#\\/tp1") + " .mjx-mrow").should(
        "contain.text",
        `(${nInDOM(Math.trunc(t1x * 1000) / 1000)}`,
      );
      cy.get(cesc("#\\/tp1") + " .mjx-mrow").should(
        "contain.text",
        `${nInDOM(Math.trunc(t1y * 1000) / 1000)}`,
      );
      cy.get(cesc("#\\/tp2") + " .mjx-mrow").should(
        "contain.text",
        `(${nInDOM(Math.trunc(t2x * 1000) / 1000)}`,
      );
      cy.get(cesc("#\\/tp2") + " .mjx-mrow").should(
        "contain.text",
        `${nInDOM(Math.trunc(t2y * 1000) / 1000)}`,
      );
      cy.get(cesc("#\\/tp3") + " .mjx-mrow").should(
        "contain.text",
        `(${nInDOM(Math.trunc(t3x * 1000) / 1000)}`,
      );
      cy.get(cesc("#\\/tp3") + " .mjx-mrow").should(
        "contain.text",
        `${nInDOM(Math.trunc(t3y * 1000) / 1000)}`,
      );
