...
describe("Spreadsheet Tag Tests", function () {
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        Array.isArray(stateVariables["/_spreadsheet1"].stateValues.cells),
      ).eq(true);
    });
    ...
    cy.window().then(async (win) => {
      ...
      for (let cellNum in cellNames) {
        for (let ind in cellNames[cellNum]) {
          expect(stateVariables[cellNames[cellNum][ind]].stateValues.text).eq(
            cellValues[cellNum],
          );
        }
      }
      ...
      for (let ssNum in spreadsheetNames) {
        for (let ssInd in spreadsheetNames[ssNum]) {
          ...
          for (let cellNum in cellLocations) {
            ...
            expect(
              stateVariables[ssName].stateValues.cells[cLoc[0] - 1][
                cLoc[1] - 1
              ],
                        ...
                        if (cellNum < 6 || ssNum == valueInd2 > 2) {
                          if (
                            stateVariables[ssName].stateValues.cells[
                              cLoc[0] - 1
                            ][cLoc[1] - 1] !== allCellValues[cellNum][valueInd2]
                          ) {
                            return false;
                          }
                          ...
                          if (ssNum === "1") {
                            ...
                            if (
                              stateVariables[ssName].stateValues.cells[
                                cLoc[0] - 1
                              ][cLoc[1] - 1] !== allCellValues[cellNum][2]
                            ) {
                              return false;
                            }
                          }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 7; ind++) {
        expect(
          stateVariables["/_spreadsheet1"].stateValues.cells[ind - 1][0],
        ).eq(`row${ind}`);
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 7; ind++) {
        expect(
          stateVariables["/_spreadsheet1"].stateValues.cells[0][ind - 1],
        ).eq(`column${ind}`);
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 8; ind++) {
        expect(
          stateVariables["/_spreadsheet1"].stateValues.cells[4][ind - 1],
        ).eq(`column${ind}`);
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let row = 1; row <= 4; row++) {
        for (let col = 1; col <= 4; col++) {
          expect(
            stateVariables["/_spreadsheet1"].stateValues.cells[row - 1][
              col - 1
            ],
      ...
      for (let row = 3; row <= 4; row++) {
        for (let col = 3; col <= 4; col++) {
          expect(
            stateVariables["/_spreadsheet2"].stateValues.cells[row - 3][
              col - 3
            ],
          ).eq(`${String.fromCharCode(64 + col)}${row}`);
        }
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 4; ind++) {
        expect(
          stateVariables["/_spreadsheet1"].stateValues.cells[2][ind - 1],
        ).eq(`column${ind}`);
      }
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 4; ind++) {
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[ind - 1][1],
        ).eq(`row${ind}`);
      }
      expect(stateVariables["/_spreadsheet1"].stateValues.cells[2][3]).eq(
        `row1`,
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let col = 1; col <= 4; col++) {
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[2][col - 1],
        ).eq(`${String.fromCharCode(64 + col)}2`);
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[3][col - 1],
        ).eq(`${String.fromCharCode(64 + col)}1`);
      }
      for (let row = 1; row <= 4; row++) {
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[row - 1][4],
        ).eq(`C${row}`);
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[row - 1][5],
        ).eq(`B${row}`);
      }
    });
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 4; ind++) {
        expect(
          stateVariables["/_spreadsheet1"].stateValues.cells[1][ind - 1],
        ).eq(`column${ind}`);
      }
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 4; ind++) {
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[ind - 1][4],
        ).eq(`row${ind}`);
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let row = 1; row <= 3; row++) {
        for (let col = 1; col <= 3; col++) {
          expect(
            stateVariables["/_spreadsheet2"].stateValues.cells[row + 1][col],
          ).eq(`${String.fromCharCode(64 + col)}${row}`);
        }
      }
    });
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 3; ind++) {
        expect(stateVariables["/_spreadsheet2"].stateValues.cells[3][ind]).eq(
          `column${ind}`,
        );
      }
    });
    ...
    cy.window().then(async (win) => {
      ...
      for (let ind = 1; ind <= 5; ind++) {
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[ind - 1][3],
        ).eq(`row${ind}`);
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let inds of cellBlockUpperLefts) {
        let row = inds[0];
        let col = inds[1];
        expect(stateVariables["/_spreadsheet2"].stateValues.cells[row][col]).eq(
          "alpha",
        );
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[row][col + 1],
        ).eq("beta");
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[row + 1][col + 1],
        ).eq("gamma");
      }
    });
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        stateVariables[
          stateVariables["/inAllCells"].activeChildren[0].componentName
        ].stateValues.xs,
      ).eqls([1, 2]);
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        stateVariables[
          stateVariables["/inAllCells"].activeChildren[1].componentName
        ].stateValues.xs,
      ).eqls([5, 4]);
      ...
      expect(
        stateVariables[
          stateVariables["/inRangeA2B4"].activeChildren[0].componentName
        ].stateValues.xs,
      ).eqls([5, 4]);
    });
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        stateVariables[
          stateVariables["/inRangeA2B4"].activeChildren[1].componentName
        ].stateValues.xs,
      ).eqls([3, 2]);
    });
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        stateVariables[
          stateVariables["/inAllCells"].activeChildren[2].componentName
        ].stateValues.xs,
      ).eqls([3, 2]);
      expect(stateVariables["/inCellB3"].activeChildren.length).eq(1);
      expect(
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        stateVariables[
          stateVariables["/inAllCells"].activeChildren[3].componentName
        ].stateValues.xs,
      ).eqls([3, 2]);
      expect(stateVariables["/inCellB3"].activeChildren.length).eq(1);
      expect(
    ...
    cy.window().then(async (win) => {
      ...
      expect(
        stateVariables[
          stateVariables["/inAllCells"].activeChildren[4].componentName
        ].stateValues.xs,
      ).eqls([3, 2]);
      expect(stateVariables["/inCellB3"].activeChildren.length).eq(1);
      expect(
      ...
      expect(
        stateVariables[
          stateVariables["/inRangeA2B4"].activeChildren[2].componentName
        ].stateValues.xs,
      ).eqls([3, 2]);
    });
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let inds of firstInds) {
        expect(
          stateVariables["/_spreadsheet1"].stateValues.cells[inds[0]][inds[1]],
        ).eq(`first`);
        expect(stateVariables["/s2"].stateValues.cells[inds[0]][inds[1]]).eq(
          `first`,
        );
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      expect(stateVariables["/s2"].replacements[0].stateValues.numColumns).eq(
        5,
      );
      for (let inds of firstInds) {
        expect(
        ...
        expect(
          stateVariables["/s2"].replacements[0].stateValues.cells[inds[0]][
            inds[1]
          ],
        ).eq(`first`);
      }
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      expect(stateVariables["/s2"].replacements[0].stateValues.numRows).eq(
        numRows,
      );
      expect(stateVariables["/s2"].replacements[0].stateValues.numColumns).eq(
        numcols,
      ...
      for (let row = 0; row < numRows; row++) {
        for (let col of [0, 1, 5]) {
          expect(
            stateVariables["/_spreadsheet1"].stateValues.cells[row][col],
          ).eq(rowTexts[col]);
          expect(
            stateVariables["/s2"].replacements[0].stateValues.cells[row][col],
          ).eq(rowTexts[col]);
        }
      }
    });
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      for (let inds of secondInds) {
        ...
        expect(
          stateVariables["/_spreadsheet2"].stateValues.cells[inds[0]][inds[1]],
        ).eq(`first`);
      }
      for (let inds of [
        ...row1Inds,
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      expect(stateVariables["/s2"].replacements[0].stateValues.cells[0][0]).eq(
        "4",
      );
      expect(stateVariables["/s2"].replacements[0].stateValues.cells[0][1]).eq(
        "4",
      );
    });
