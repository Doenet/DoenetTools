...
export function setupScene({ attributes }) {
...
export function runTests({ center, vertex1, nVertices, conservedWhenChangeNvertices = "radius", abbreviated = false }) {
  ...
  cy.window().then(async (win) => {
    ...
    if (!abbreviated) {
      ...
      cy.window().then(async (win) => {
        ...
        cy.get(cesc("#\\/mins") + " textarea").type(`{end}-1{enter}`, { force: true })
