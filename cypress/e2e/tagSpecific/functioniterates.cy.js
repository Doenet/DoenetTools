...
describe("FunctionIterates Tag Tests", function () {
  ...
  `,
    ...
    cy.window().then(async (win) => {
      ...
      expect(stateVariables["/l1/iter"].stateValues.value).eqls([
        "*",
        3,
        "a",
        "v",
      ]);
      expect(stateVariables["/l2/iter"].stateValues.value).eqls([
        "*",
        3,
        "v",
        ["^", "a", 2],
    ...
    cy.window().then(async (win) => {
      ...
      expect(stateVariables["/l5/iter"].stateValues.value).eqls([
        "+",
        ["*", 5, "q"],
        "w",
      ]);
  ...
  <p>Iterates: <aslist>$fis.iterates{name="iterates"}</aslist></p>
  ...
  `,
    ...
    cy.get(cesc("#\\/x") + " textarea").type("{end}{backspace}q{enter}", {
      force: true,
    });
    cy.get(cesc("#\\/y") + " textarea").type("{end}{backspace}r{enter}", {
      force: true,
    });
  ...
  `,
    ...
    cy.get(cesc("#\\/vars") + " textarea").type("{end}, z{enter}", {
      force: true,
    });
    ...
    cy.get(cesc("#\\/u") + " textarea").type("{end}{leftArrow}, -4{enter}", {
      force: true,
    });
  ...
  `,
    ...
    cy.get(cesc("#\\/u") + " textarea").type("{end}{leftArrow}a{enter}", {
      force: true,
    });
    ...
    cy.log(`add component to function`);
    cy.get(cesc("#\\/u") + " textarea").type("{backspace}{enter}", {
      force: true,
    });
    cy.get(cesc("#\\/fformula") + " textarea").type(
      "{end}{leftArrow}z, x-z{enter}",
