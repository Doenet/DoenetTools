...
$learnerUsername =  mysqli_real_escape_string($conn,$_REQUEST["learnerUsername"]);
...
if ($legitAccessKey == 1){
  ...
  $sql = "
  SELECT timeLimitOverride,
  Minute(timeLimitOverride) AS minuteLimit,
  Hour(timeLimitOverride) AS hourLimit
  FROM user_assignment 
  WHERE assignmentId = '$assignmentId'
  AND username = '$learnerUsername'
  ";
  $result = $conn->query($sql);

  if ($result->num_rows > 0){
    $row = $result->fetch_assoc();
    if ($row["timeLimitOverride"] != NULL){
      $minuteLimit = $row["minuteLimit"];
      $hourLimit = $row["hourLimit"];
    }
  }
